...
# Press button 1 while holding button 2 to advance manually
...
if __name__ == "__main__":
    ...
    my_oled = oled_wrapper()
    my_oled.han_tyumi()
    
    state = 0
    while True:
        if b2.value() == 1:
            state = step(b1.value(), state)
        else:
            state = step(ain.read_voltage() / 10, state)
